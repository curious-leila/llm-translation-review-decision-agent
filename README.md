...
| 目标用户 | 人工复核资源有限的中小全球化内容团队                    |
| 核心场景 | EN 到 zh-CN 的营销文案、产品界面与客服话术译文审校         |
| 当前状态 | 可本地运行与部署的产品原型                          |
...
> 为营销、客服与界面文案译文建立可信、可审计的审校决策层：用户提交原文与候选译文，依据质量风险与证据状态，将译文分流至自动通过、抽样复核或人工复核，在降低复核成本的同时守住错误放行的安全底线。
...
当前阶段以产品原型、冻结真实运行与工程验收为主，不使用未经验证的业务效果数据。

已完成的验证包括：

- 产品链路可用：公开 Demo、冻结案例回放与 Live Review 均已验证，静态浏览不依赖后端启动；
- 成功闭环可回放：UI-003 完整走通“外部证据需求 → 官方来源 HIT → 证据准入 → 充分性成立 → 术语复评 → AUTO_PASS”；
- 失败路径可控：MKT-020 在证据不足时安全弃权并转入人工复核，未伪造成功结果；
- Agent 停止规则有效：历史案例在证据充分后的工具调用由 4 次降至 1 次；
- 工程验收通过：124 个自动化测试覆盖接口、证据包完整性、证据链、审校工作流与页面可读性。
当前结果证明决策机制按设计运行，但不代表生产效果或统计显著提升。
...
├── src/review_triage/              # 质量判断、证据查证与分流逻辑
├── tests/                          # 自动化测试
...
- 接入经人审验证的案例记忆，在严格准入与冲突检查下支持相似术语案例复用；
