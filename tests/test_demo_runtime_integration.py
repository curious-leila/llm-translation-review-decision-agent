...
class DemoStaticPresentationTests(unittest.TestCase):
    ...
    def test_landing_is_an_hr_first_replay_workbench(self) -> None:
        ...
        self.assertIn('<span class="hero-title-line">让每条译文，</span>', index)
        self.assertIn('<span class="hero-title-line">走到合适的审校路径</span>', index)
        self.assertNotIn("让每条译文，走到合适的审校路径。", index)
        self.assertIn("还是交给人工</p>", index)
        ...
        self.assertIn('<p class="eyebrow">HOW THE AGENT WORKS</p>', index)
        self.assertIn('<h2 id="workspace-title">直接看 Agent 如何做出审校决策</h2>', index)
        self.assertNotIn("直接试玩一次真实决策", index)
        self.assertIn("真实运行回放 · 点击切换案例", index)
        self.assertNotIn("产品可信能力", index)
        ...
        self.assertIn('class="nav-pill nav-pill-yellow is-active"', index)
        self.assertIn('class="nav-pill nav-pill-blue"', index)
        self.assertIn('class="nav-pill nav-pill-mint"', index)
        self.assertIn("在线 Review Agent", index)
        self.assertIn("提交译文，调用真实后端", index)
        self.assertIn(".hero-live-label,.hero-live-note{display:none}", styles)
        self.assertIn('class="case-workspace-card"', index)
        self.assertIn('id="mobile-case-picker"', index)
        self.assertIn('<small>当前案例</small>', index)
        self.assertIn('class="mobile-case-action">切换案例', index)
        self.assertIn('.case-switcher.is-mobile-open{display:grid}', styles)
        self.assertIn('(max-width:900px) and (max-height:500px)', styles)
        self.assertIn('(max-width: 900px) and (max-height: 500px)', app)
        self.assertIn('setMobileCasePicker(false)', app)
        self.assertIn(".case-tab:not(.is-active)", styles)
        self.assertIn('>开始审校</button>', index)
        self.assertNotIn('>方法与架构</a>', index)
        self.assertNotIn('>粘贴自己的译文</button>', index)
        self.assertIn("提交自己的译文", index)
        self.assertIn("结果会在当前三栏工作台中展示", index)
        self.assertIn("提交成功，Review Agent 正在分析", index)
        self.assertIn("本次审校已安全停止，未生成任何结论。", app)
        self.assertIn('form.setAttribute("aria-busy", String(submitting))', app)
        self.assertIn("前置评测用于形成冻结路由策略，不是当前 Agent 上线后的效果指标。", index)
        self.assertIn('role="tablist"', index)
        self.assertIn('role="status"', index)
        self.assertIn('id="case-tab-direct" class="case-tab is-active"', index)
        self.assertIn('aria-labelledby="case-tab-direct"', index)
        self.assertIn('data-active-case="1"', index)
        self.assertIn('<strong id="mobile-case-label">无需查证</strong>', index)
        self.assertIn('name="content_type" value="MARKETING" required checked', index)
        self.assertIn('name="content_type" value="UI"><span>界面文案</span>', index)
        self.assertEqual(index.count("data-default-replay"), 1)
        ...
        self.assertIn(".hero-live-cta{display:grid", styles)
        self.assertIn("--control-shadow:", styles)
        self.assertIn(".nav-pill:hover{transform:translateY(-3px)", styles)
